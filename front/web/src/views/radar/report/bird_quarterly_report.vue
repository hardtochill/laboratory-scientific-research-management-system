<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="dataList">
      ...
      <el-table-column prop="startDate" label="季度" align="center">
        <template #default="scope">
          {{ subData(scope.row.startDate) }}
        </template>
      </el-table-column>
    ...
    <el-dialog title="详情" v-model="open" width="80%" append-to-body>
      <el-descriptions
          ...
          border
      >
        <el-descriptions-item label="季度" :span="3">
          {{ subData(detailsData.startDate) }}
      ...
      <el-table v-loading="loading" :data="detailsData.quarterlyReportTimeList">
...
<script setup>
import {getQuarterlyReport, listQuarterlyReport} from "@/api/radar/quarterlyReport.js";
...
const getList = () => {
  loading.value = true
  listQuarterlyReport(queryParams.value).then(response => {
...
function handleDetails(row) {
  getQuarterlyReport(row.id).then(res => {
...
function handleExport(row) {
  proxy.download("quarterlyReport/export", {
    id: row.id,
  }, `鸟情季度报_${subData(row.startDate)}.xlsx`)
}
