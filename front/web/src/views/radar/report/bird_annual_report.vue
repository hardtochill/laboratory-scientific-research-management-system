  ...
  <div class="app-container">
    ...
    <el-dialog title="详情" v-model="open" width="80%" append-to-body>
      <el-descriptions
          ...
          border
      >
        <el-descriptions-item label="年份" :span="3">
      ...
      <el-table v-loading="loading" :data="detailsData.annualReportTimeList">
...
<script setup>
import {getAnnualReport, listAnnualReport} from "@/api/radar/annualReport.js";
...
function handleExport(row) {
  proxy.download("annualReport/export", {
    id: row.id,
  }, `鸟情年报_${subData(row.startDate)}.xlsx`)
}
...
const getList = () => {
  loading.value = true
  listAnnualReport(queryParams.value).then(response => {
...
function handleDetails(row) {
  getAnnualReport(row.id).then(res => {
