<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="dataList">
      ...
      <el-table-column prop="startDate" label="月份" align="center">
    ...
    <el-dialog title="详情" v-model="open" width="80%" append-to-body>
      <el-descriptions
          ...
          border
      >
        <el-descriptions-item label="月份" :span="3">
      ...
      <el-table v-loading="loading" :data="detailsData.monthlyReportTimeList">
...
<script setup>
import {getMonthlyReport, listMonthlyReport} from "@/api/radar/monthlyReport.js";
...
const getList = () => {
  loading.value = true
  listMonthlyReport(queryParams.value).then(response => {
...
function handleExport(row) {
  proxy.download("monthlyReport/export", {
    id: row.id,
  }, `鸟情月报_${subData(row.startDate)}.xlsx`)
}

function handleDetails(row) {
  getMonthlyReport(row.id).then(res => {
