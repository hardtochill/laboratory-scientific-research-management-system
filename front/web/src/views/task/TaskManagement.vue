...
<script setup>
import { ref, onMounted, onUnmounted, reactive, toRefs, computed, nextTick } from 'vue'
...
// 显示任务汇报对话框
// 处理任务汇报按钮点击
const handleTaskReportClick = (task) => {
  showTaskReportDialog(task)
}

const showTaskReportDialog = (task) => {
...
const loadTaskReports = async () => {
  if (!currentTaskForReport.value) return
  ...
  try {
    const params = {}
    if (reportQuery.dateRange && reportQuery.dateRange.length === 2) {
      if (reportQuery.dateRange[0]) {
        params.startTime = reportQuery.dateRange[0]
      }
      if (reportQuery.dateRange[1]) {
        params.endTime = reportQuery.dateRange[1]
      }
    }
    const response = await getTaskReportList(currentTaskForReport.value.taskId, params)
    // 使用 nextTick 确保数据更新后视图刷新
    await nextTick()
...
const handleReportDialogClose = () => {
  ...
  currentTaskForReport.value = null
  reportList.value = []
